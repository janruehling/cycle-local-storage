...
import { getPlansId$, putPlansId$ } from 'Remote'
...
export default sources => {
  ...
  const editRequest$ = mergeOrFlatMapLatest('formData$', page$)
    .combineLatest(plan$)
    .sample(saveClick$)
    .map(([formData, plan]) => filterChangedFields(formData, plan))
    .flatMap(formData => putPlansId$({
      ...sources,
  ...
  const editResponse$ = sources.responses$
    .filter(res$ => res$ && res$.request)
    .filter(res$ => res$.request.category === 'putPlansId$')
  ...
  const children = [header, search, appShell, page$, planReq, editReq]
